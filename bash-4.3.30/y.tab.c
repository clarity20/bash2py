...
{
       0,   440,   440,   451,   460,   475,   485,   487,   491,   497,
     503,   509,   515,   521,   527,   533,   539,   545,   551,   557,
     563,   569,   575,   581,   588,   595,   602,   609,   616,   623,
     629,   635,   641,   647,   653,   659,   665,   671,   677,   683,
     689,   695,   701,   707,   713,   719,   725,   731,   737,   743,
     749,   755,   763,   769,   775,   783,   787,   798,   805,   814,
     816,   818,   834,   836,   840,   842,   844,   851,   858,   860,
     862,   864,   866,   868,   870,   874,   884,   894,   904,   914,
     924,   934,   944,   956,   966,   976,   986,   998,  1008,  1018,
    1028,  1038,  1048,  1060,  1070,  1080,  1092,  1101,  1110,  1120,
    1122,  1153,  1160,  1169,  1190,  1199,  1220,  1231,  1237,  1243,
    1252,  1256,  1264,  1268,  1277,  1286,  1297,  1298,  1302,  1304,
    1306,  1308,  1312,  1314,  1316,  1318,  1320,  1322,  1326,  1328,
    1337,  1345,  1346,  1352,  1353,  1360,  1364,  1366,  1368,  1375,
    1377,  1379,  1383,  1384,  1387,  1396,  1405,  1416,  1417,  1426,
    1439,  1455,  1470,  1472,  1474,  1481,  1484,  1488,  1490,  1499,
    1508,  1530,  1554,  1556,  1579,  1583,  1591,  1599
};
#endif
...
#line 504 "./parse.y"
    {
			  source.dest = VALUE((yyvsp[(1) - (3)].number));
...
#line 642 "./parse.y"
    {
			  source.dest = 0;
			  redir.dest = VALUE((yyvsp[(2) - (2)].number));
...
#line 648 "./parse.y"
    {
			  source.dest = VALUE((yyvsp[(1) - (3)].number));
			  redir.dest = VALUE((yyvsp[(3) - (3)].number));
			  (yyval.redirect) = make_redirection (source, r_duplicating_input, redir, 0);
			}
    break;
...
#ifdef BASH2PY
              (yyval.element).position = (yyvsp[(1) - (1)].word)->position;
#endif
            }
    break;
...
#ifdef BASH2PY
              (yyval.element).position = (yyvsp[(1) - (1)].redirect)->position;
#endif
            }
    break;
...
#line 875 "./parse.y"
    {
			  (yyval.command) = make_for_command ((yyvsp[(2) - (6)].word), add_string_to_list ("\"$@\"", (WORD_LIST *)NULL
#ifdef BASH2PY
        , &((yyvsp[(2) - (6)].word)->position)), (yyvsp[(5) - (6)].command), &((yyvsp[(1) - (6)].position)));
#else
        ), (yyvsp[(5) - (6)].command), word_lineno[word_top]);
...
#line 895 "./parse.y"
    {
			  (yyval.command) = make_for_command ((yyvsp[(2) - (7)].word), add_string_to_list ("\"$@\"", (WORD_LIST *)NULL
#ifdef BASH2PY
        , &((yyvsp[(2) - (7)].word)->position)), (yyvsp[(6) - (7)].command), &((yyvsp[(1) - (7)].position)));
#else
        ), (yyvsp[(6) - (7)].command), word_lineno[word_top]);
			  if (word_top > 0) word_top--;
#endif
			}
    break;
...
#line 957 "./parse.y"
    {
				  (yyval.command) = make_arith_for_command ((yyvsp[(2) - (7)].word_list), (yyvsp[(6) - (7)].command), 
#ifdef BASH2PY
                                                      &((yyvsp[(1) - (7)].position)));
#else
                                                      arith_for_lineno);
...
#line 977 "./parse.y"
    {
				  (yyval.command) = make_arith_for_command ((yyvsp[(2) - (5)].word_list), (yyvsp[(4) - (5)].command), 
#ifdef BASH2PY
                                                      &((yyvsp[(1) - (5)].position)));
#else
                                                      arith_for_lineno);
...
#line 999 "./parse.y"
    {
			  (yyval.command) = make_select_command ((yyvsp[(2) - (6)].word), add_string_to_list ("\"$@\"", (WORD_LIST *)NULL 
#ifdef BASH2PY
            ,&((yyvsp[(2) - (6)].word)->position)), (yyvsp[(5) - (6)].command), &((yyvsp[(1) - (6)].position)));
#else
            ), (yyvsp[(5) - (6)].command), word_lineno[word_top]);
...
#line 1019 "./parse.y"
    {
			  (yyval.command) = make_select_command ((yyvsp[(2) - (7)].word), add_string_to_list ("\"$@\"", (WORD_LIST *)NULL
#ifdef BASH2PY
            , &((yyvsp[(2) - (7)].word)->position)), (yyvsp[(6) - (7)].command), &((yyvsp[(1) - (7)].position)));
#else
           ), (yyvsp[(6) - (7)].command), word_lineno[word_top]);
...
#line 1039 "./parse.y"
    {
			  (yyval.command) = make_select_command ((yyvsp[(2) - (10)].word), REVERSE_LIST ((yyvsp[(5) - (10)].word_list), WORD_LIST *), (yyvsp[(9) - (10)].command), 
#ifdef BASH2PY
                                        &((yyvsp[(1) - (10)].position)));
#else
                                        word_lineno[word_top]);
...
/* Line 1806 of yacc.c  */
#line 1093 "./parse.y"
    { (yyval.command) = make_function_def ((yyvsp[(1) - (5)].word), (yyvsp[(5) - (5)].command), 
#ifdef BASH2PY
                                             &((yyvsp[(1) - (5)].word)->position)
#else
...
/* Line 1806 of yacc.c  */
#line 1102 "./parse.y"
    { (yyval.command) = make_function_def ((yyvsp[(2) - (6)].word), (yyvsp[(6) - (6)].command), 
#ifdef BASH2PY
                                             &((yyvsp[(1) - (6)].position))
#else
                                             function_dstart, function_bstart
...
/* Line 1806 of yacc.c  */
#line 1111 "./parse.y"
    { (yyval.command) = make_function_def ((yyvsp[(2) - (4)].word), (yyvsp[(4) - (4)].command), 
#ifdef BASH2PY
                                              &((yyvsp[(1) - (4)].position))
#else
                                              function_dstart, function_bstart
...
#line 1500 "./parse.y"
    {
			  if ((yyvsp[(2) - (2)].command))
			    (yyvsp[(2) - (2)].command)->flags |= VALUE((yyvsp[(1) - (2)].number));
			  (yyval.command) = (yyvsp[(2) - (2)].command);
#ifdef BASH2PY
...
#ifdef BASH2PY
                                                          ,&((yyvsp[(1) - (2)].number).position)
#endif
                                       );
              (yyval.command)->flags |= VALUE((yyvsp[(1) - (2)].number));
			  /* XXX - let's cheat and push a newline back */

			  if (VALUE((yyvsp[(2) - (2)].number)) == '\n')
			    token_to_read = '\n';
			}
    break;
